...
function acaoConcluir() {
    ...
    let spanTarefa= li.children[2] // ALTEREI: o span é o terceiro elemento já que os botões de ordenação foram adicionados
    // console.log(spanTarefa.classList)
    ...
    if (spanTarefa.classList.length > 1) {
        ...
        let p = li.firstElementChild; //ALTEREI: para passar por todos os botoes, passa por todos elemntos da lista agora menos o que tem classe de tarefa
        while (p != null) {
            if(p.classList != "tarefa"){
...
function acaoEditar() {
    if (buffer == null) {
        ...
        // Passo 2, acima
        let spanTarefa= li.children[2];  // ALTEREI: Para pegar o span, já que agora tem dois botões antes dele
...
function acaoAtualizar() {
    ...
    if (texto != '') {
        // Passo 2, acima
        let spanTarefa= buffer.children[2]; // ALTEREI: Para pegar o span, já que agora tem dois botões antes dele
...
function acaoRemover() {
    let li = this.parentNode;
    let lista = li.parentNode;
    let span = li.children[2]; // ALTEREI: Para pegar o span, já que agora tem dois botões antes dele
...
function acaoSubir() {
    let li = this.parentNode;
    let lista = li.parentNode;
    if(li.previousElementSibling != null){ // Se não for o primeiro elemento da lista
        lista.insertBefore(li, li.previousElementSibling); //troca
    }
    else{
        console.log('O item já está na primeira posição da lista.');
    }
}
...
function acaoDescer() {
    let li = this.parentNode;
    let lista = li.parentNode;
    if(li.nextElementSibling != null){ //se não for o último da lista
        lista.insertBefore(li.nextElementSibling, li); //troca
    }
    else{
        console.log('O item já está na última posição da lista.');
    }
}
...
function criaLi(texto) {
    ...
    btnSubir.classList.add('subir');
    btnSubir.addEventListener('click', acaoSubir);

    btnDescer.classList.add('descer');
    btnDescer.addEventListener('click', acaoDescer);
