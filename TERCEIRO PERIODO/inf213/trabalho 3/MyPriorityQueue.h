...
//esse arquivo é praticamente o mesmo da prática porém eu alterei de maneira que essa fila de prioridade funcionasse de maneira invertida, ou seja, que o elemento de menor prioridade fosse o primeiro
// nn esta 95% certo
...
void MyPriorityQueue<T>::moveDown(int pos) { //refaz o heap da posicao "pos" em diante
	while(2*pos+1 < heap.size()) { //enquanto a posicao atual tiver pelo menos um filho...
		...
		if(heap[pos] < heap[menorFilho]) { 
			...
			swap(heap[pos],heap[menorFilho]); //troque o atual com o menor filho 
			pos = menorFilho; //repita o processo agora na posicao "menorFilho"
		}
	}
}
...
void MyPriorityQueue<T>::moveUp(int pos) { 
	while(pos>=0) {
		...
		if(heap[pos] < heap[pai]) { //mudei aqui, visto que o pai tem que ser o menor e nao o maior para nossa fila de prioridade funcionar de maneira
