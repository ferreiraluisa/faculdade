...
/* PRIMEIRO METODO -----------------------------------------------------------------------------
usei com o vector como variavel global, tentei fazer de outra forma aproveitando os recursos da recursividade*/
vector <int> reserva;
int buscaBin(int *array,int begin, int end, int chave) {
	...
	if (begin > end){
		int meio_reserva = reserva.front();
		reserva.clear();
		return meio_reserva;
	}
	...
	if (array[meio] > chave){
		reserva.clear();
		reserva.push_back(meio);
		return buscaBin(array,begin, meio-1, chave);
	}
	if (array[meio] == chave) {
		if(begin==end) return meio;
		else return buscaBin(array,begin,meio,chave);
	}
...
/*
botei no submitty mas nao passou em todos os testes
int buscaBin(int *array,int begin, int end, int chave) {
	if (begin > end) 
		if(array[begin-1]<chave) return -1;
		else return buscaBin(array,end,begin-1,chave+1);
	int meio = (end-begin)/2 + begin;
	if (array[meio] == chave) {
		if(begin==end) return meio;
		else return buscaBin(array,begin,meio,chave);
