...
void reverse(MyList2<T> &lista) {
    MyList2<int>::iterator it1 = lista.begin();
    int size = 0;

    for(MyList2<int>::iterator it = lista.begin();it!=lista.end();it++){ //para pegar o iterador que aponta pro ultimo elemeto da lista ja que lista.end() retorna null
        it1 = it; 
        size++;
    }
    MyList2<int>::iterator it = lista.begin();
    for(int i=0;i<size/2;i++){ //tamanho/2 porque vai trocar o primeiro pelo ultimo, segundo pelo penultimo, etc..
      T aux = *it;
      *it = *it1;
      *it1 = aux;
      it++;
