...
private:
	...
	Node<T> * copyNodes(const Node<T> *orig) const;
...
//operador de cópia
template<class T>
Node<T> * MySet<T>::copyNodes(const Node<T> *orig) const{
	if(orig == NULL) return NULL;
	else{
		Node<T> *copia = new Node<T>(orig->elem);
		copia->left = copyNodes(orig->left);
		copia->right = copyNodes(orig->right);
		return copia;
	}
